...
static const AVOption object_tracker_options[] = {
        ...
        {"angle_filter", "object filter based on the angle", OFFSET(angle), AV_OPT_TYPE_DOUBLE, {.dbl = 0}, 0, 360, FLAGS },
...
static void draw_object_arrow(Object *obj, AVFrame *frame){
    ...
    double angle_rad = math_angle * (PI / 180);
    for(int i = 0; i < 5; i++){
...
static void check_object(Object *obj, TDContext *s, AVFrame *frame)
{
    ...
    if ((s->angle && obj->average_angle >= min_angle && obj->average_angle <= max_angle) || !s->angle) {  // Filter by angle where the object goes
        if(s->print_rectangles_position || s->object_marker_box) // if we dont need rectangles, do not search for it.
...
static int config_input(AVFilterLink *inlink)
{
    ...
    if (s->parameters) { 
        ...
        if (s->url) {
            ...
            s->bytes = sprintf(s->buffer, (const char *)parameters, version, release_date, s->tripwire, s->start_x, s->start_y, s->tripwire_line_angle, s->tripwire_marker_line,
            ...
            s->min_mv, s->angle, s->angle_range, s->object_marker_box, s->thickness, s->object_marker_box_history, s->detection_threshold,
            ...
            s->parameters, s->grid_size, s->obj_survival_time, s->max_obj_distance_history, s->scene_static_frames);
        ...
        if (s->std_err_text_output_enable) {  // If the error log or URL is enabled, send the data there
            printf(parameters, version, release_date, s->tripwire, s->start_x, s->start_y, s->tripwire_line_angle, s->tripwire_marker_line,
            s->max_distance, s->min_mv_length, s->max_angle_diff, s->crop_x, s->crop_y, s->crop_width, s->crop_height, s->resize_to_crop, s->black_filter,
            s->min_mv, s->angle, s->angle_range, s->object_marker_box, s->thickness, s->object_marker_box_history, s->detection_threshold,
            s->mask_static_areas, s->mask_i_frames, s->keep_mask_on_image, s->line_break,
            s->print_only_intersect_trigger, s->print_lite_mode, s->print_rectangles_position, s->url, s->std_err_text_output_enable,
...
static void mask_image(Object** object_list, int object_counter, AVFrame *frame, TDContext *s)
{
    ...
    for (int i=0; i < object_counter; i++){
        if (object_list[i]->counter > s->min_mv) {
...
static int filter_frame(AVFilterLink *inlink, AVFrame *frame) {
    ...
    if (motion_vector_table) {  // If the frame has mvs.
        ...
        for (i = 0; i < mv_table_size; i++) {   // Loop trough all the mvs of the frame
            ...
            if (s->black_filter){
                if (mv->src_x > frame->width){
                    continue;
                }
                if (mv->src_y > frame->height){
                    continue;
                }
